...
public class statsVM implements Runnable {

	// static final String SERVER_NAME = "130.65.132.14";
	static final String SERVER_NAME = "130.65.133.77";
	...
	private static final int SELECTED_COUNTER_ID = 6; // Active (mem) in KB
														// (absolute)
	static Integer[] a = { 125, 24, 6, 143, 156 };
	static String[] aName = { "disk", "mem", "cpu", "net", "sys" };
	private HashMap<String, String> infoList = new HashMap<String, String>();
	int counter = 0;
	static writetofile writer;
	Gson gson = new Gson();

	public statsVM(String hostIP) throws IOException {
		this.ip = hostIP;
		...
		try {
			...
			while (true) {
				...
				try {
					...
					System.out.println("total queries possible "
							+ queryAvailablePerfMetric.length);
				...
				try {
					...
					DBObject basicdbObject = (DBObject) JSON.parse(gson
							.toJson(infoList));
